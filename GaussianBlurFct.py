        ...
        for i in range(0, img_row):
            for j in range(0, img_col):
                ...
                for k in range(0, kern_row):
                    for l in range(0, kern_col):  # to go through each pixel of the kernel_zone
                        blured_pxl_val += (kernel_zone[k, l] * kernel[k, l]) / np.sum(
                            kernel)  # compute the mean of the pixel's color value around a pixel with a certain
                        # weight for each pixel determined by the kernel
