...
def otsu_threshold(hist, img):
    ...
    while threshold_try < 256 and (threshold_try < 50 or i_min != threshold_try-3): #prevent to try threshold beyond 255 and stop when the indice of the minimum variation is found
        ...
        foreground = hist[threshold_try:] #the pixels that should be white according to the threshold that is being tried (above the threshold)
        
        #computing weights
        w_background = np.sum(background)/nb_pixels #compute the weight of every pixel's color value below the threshold which is being tried
        w_foreground = np.sum(foreground)/nb_pixels #compute the weight of every pixel's color value above the threshold which is being tried
        
        #computing mean
        mean_background = np.sum([pixel_value*nb_appearance for pixel_value, nb_appearance in enumerate(background)])/np.sum(background) #compute the mean of every pixel's color value below the threshold which is being tried multiplied by their number of appearance
        mean_foreground = np.sum([pixel_value*nb_appearance for pixel_value, nb_appearance in enumerate(foreground)])/np.sum(foreground) #compute the mean of every pixel's color value below the threshold which is being tried multiplied by their number of appearance
        
        #computing variance
        var_background = np.nan_to_num(np.sum([(pixel_value-mean_background)**2*nb_appearance for pixel_value, nb_appearance in enumerate(background)])/np.sum(background)) #compute the variance of the background
        var_foreground = np.nan_to_num(np.sum([(pixel_value-mean_foreground)**2*nb_appearance for pixel_value, nb_appearance in enumerate(foreground)])/np.sum(foreground)) #compute the variance of the foreground
